 ...
 * <<CONTROL>>
 * 
 * Class that sorts and interprets the obtained data in order to
 * execute the method able to retrieve the correct results
 * based on the user's choice 
 * 
 * Classe che smista e interpreta i dati ottenuti in modo
...
public final class Controller {
	 ...
	 * 
	 * Metodo principale della classe. Assolve le responsabilità della classe
	 *
	...
	public void Control(String[] args) throws FileNotFoundException,
		...
		List<Long[]> res= soq.getResults(job, query); //Filling the list with the values obtained from the query
		ut.shareSheet(spid);						  //Makes available the access to the spreadsheet to anyone with the link
		ut.getSheetByTitle(spid);					  
		ut.writeSheet(spid, res, query);			  //Writes the results contained in the 'res' list on the Google spreadsheet
	}
}